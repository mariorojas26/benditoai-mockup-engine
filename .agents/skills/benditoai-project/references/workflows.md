...
- Campaign UI is a 7-screen dark BenditoAI wizard: focus/model route, product info plus up to 3 product reference images, optional model confirmation, visual presets, marketing copy, output formats, and final export.
...
- Endpoint accepts product image references, palette/tone/style/background/copy settings, selected formats, and optional model/outfit references. It uses `benditoai_call_gemini_campaign()` for campaign-specific aspect ratio/image size and builds different prompts for with-model vs no-model campaigns.
