...
document.addEventListener("submit", async function (event) {
    if (event.target.id !== "benditoai-remove-bg-form") return;
    event.preventDefault();

    const formData = new FormData(event.target);
    formData.append("action", "benditoai_remove_background");

    const result = document.getElementById("benditoai-remove-bg-result");
    if (!result) return;
    ...
    window.BenditoAIUX?.preview?.loading(stage, { label: "Eliminando fondo..." });
    ...
    try {
        ...
        const message = window.BenditoAIUX?.getErrorMessage(payload, "No se pudo eliminar el fondo.");
        window.BenditoAIUX?.preview?.error(stage, {
            title: "No se pudo eliminar el fondo",
            message
        });
    } catch (_error) {
        window.BenditoAIUX?.preview?.error(stage, {
